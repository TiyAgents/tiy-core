...
Providers that inject API keys, compat settings, and/or custom base URLs, then delegate to an existing protocol. Most are generated by macros in `protocol::delegation`.
...
```
provider/
├── mod.rs           # Module declarations, re-exports protocol infrastructure
...
Use the macros in `protocol::delegation` to generate a provider:
...
注入 API Key、兼容性设置和/或自定义 Base URL 后委托给现有协议的提供商。大多数通过 `protocol::delegation` 中的宏生成。
...
使用 `protocol::delegation` 中的宏生成提供商：
